...
} from "https://esm.sh/solid-js@^1.9.15";
import {
  addEventListener as addEventListener$1,
  className,
  ...
  template,
  use
} from "https://esm.sh/solid-js@^1.9.15/web";
...
import { createComponent, isServer, memo, mergeProps } from "https://esm.sh/solid-js@^1.9.15/web";
...
var objIsEqual = (obj1, obj2, isEqual3 = (value1, value2) => value1 === value2) => objSize(obj1) === objSize(obj2) && objEvery(
  obj1,
  (value1, index) => objHas(obj2, index) ? isObject(value1) ? (
    /* istanbul ignore next */
    isObject(obj2[index]) ? objIsEqual(obj2[index], value1, isEqual3) : false
  ) : isEqual3(value1, obj2[index]) : false
);
...
var useListenable = (listenable, thing, returnType, listenerArgGetters = EMPTY_LISTENER_ARG_GETTERS) => {
  ...
  const getResult = (resolvedThing, listenerArguments) => resolvedThing?.[(returnType == 6 ? _HAS : GET) + listenable]?.(...listenerArguments) ?? DEFAULTS[returnType];
  const updateResult = (resolvedThing = getThing(thing), listenerArguments = getListenerArguments()) => {
...
var argsOrGetArgs = (args, store, parameter) => arrayMap(
  args,
  (arg) => isFunction(arg) ? isEmpty(arg) ? getThing(arg) : arg(parameter, store) : arg
);
...
var useCreatePersister = (store, create, then, destroy) => {
  ...
  createEffect(() => {
    ...
    const resolvedStore = getThing(store);
    const destroyPersister = (persister2) => tryFinallyAsync(
      () => persister2.destroy(),
      () => destroy?.(persister2)
    );
    void tryCatch(async () => {
...
var size2 = (thing) => thing.length;
var isEmpty2 = (thing) => size2(thing) == 0;
...
var arrayClear = (array, to = size2(array)) => array.splice(0, to);
...
var objHas2 = (obj, id2) => object2.hasOwn(obj, id2);
...
var objEvery2 = (obj, cb) => {
  for (const id2 in obj) {
    if (objHas2(obj, id2) && !cb(obj[id2], id2)) {
      return false;
    }
  }
  return true;
};
var objToArray = (obj, cb) => arrayMap2(objEntries(obj), ([id2, value]) => cb(value, id2));
var objMap = (obj, cb) => {
  const mapped = objNew();
...
var objIsEqual2 = (obj1, obj2, isEqual3 = (value1, value2) => value1 === value2) => objSize2(obj1) === objSize2(obj2) && objEvery2(
  obj1,
  (value1, index) => objHas2(obj2, index) ? isObject2(value1) ? (
    /* istanbul ignore next */
    isObject2(obj2[index]) ? objIsEqual2(obj2[index], value1, isEqual3) : false
  ) : isEqual3(value1, obj2[index]) : false
);
var objValidate = (obj, validateChild, onInvalidObj) => {
  if (isNullish2(obj) || !isObject2(obj) || objIsEmpty(obj) || objFrozen(obj)) {
    onInvalidObj?.();
    return false;
  }
...
var tryCatch2 = async (action, onError) => {
  try {
    return await action();
  } catch (error) {
    onError?.(error);
...
var tryCatchIgnore = (action, onIgnoredError) => tryCatch2(action, (error) => void tryCatch2(() => onIgnoredError?.(error)));
...
var tryFinallyAsync2 = async (action, finallyAction) => {
  try {
    return await action();
  } finally {
    await finallyAction();
...
var mapMatch = (map2, obj, set, del = mapSet) => {
  ...
  mapForEach(map2, (id2) => objHas2(obj, id2) ? 0 : del(map2, id2));
  return map2;
};
...
var createCustomPersister = (store, getPersisted, setPersisted, addPersisterListener, delPersisterListener, onIgnoredError, persist, extra = {}, isSynchronizer = 0, scheduleId = []) => {
  ...
  const run = async () => {
    if (!mapGet(scheduleRunning, scheduleId)) {
      ...
      let actionIndex = 0;
      await tryFinallyAsync2(
        async () => {
          while (actionIndex < size2(scheduledActions)) {
  ...
  const pruneSchedule = () => {
    if (!mapGet(scheduleReferences, scheduleId) && !mapGet(scheduleRunning, scheduleId) && isEmpty2(mapGet(scheduleActions, scheduleId) ?? [])) {
  ...
  const setContentOrChanges = (contentOrChanges) => {
    (isMergeableStore && isArray2(contentOrChanges?.[0]) ? contentOrChanges?.[2] === 1 ? store.__[4] : store.__[3] : contentOrChanges?.[2] === 1 ? store._[10] : store._[9])(contentOrChanges);
  };
  ...
  const trackAutoLoadPromise = (newAutoLoadPromise) => tryFinallyAsync2(
  ...
  const startAutoLoad = async (initialContent) => {
    ...
    if (generation == autoLoadGeneration && !destroyed) {
      const listenerPromise = tryCatch2(async () => {
      ...
      tryFinallyAsync2(
        ...
        async () => {
          if (!isUndefined2(handle)) {
            await tryCatch2(() => delPersisterListener(handle), onIgnoredError);
          }
        }
      )
    );
  ...
  const schedule = async (...actions) => {
    ...
    const completion = promiseNew(
      (resolve) => !isEmpty2(actions) ? arrayPush(
        mapGet(scheduleActions, scheduleId),
        ...arrayMap2(actions, (action, index) => {
          const last = index == size2(actions) - 1;
          return [
            async () => {
              ...
              await tryFinallyAsync2(
                async () => {
                  try {
                    await tryCatch2(action, onIgnoredError);
                  } catch (error) {
                    if (!failed) {
                      failed = true;
                      firstError = error;
  ...
  const destroy = async () => {
    if (isUndefined2(destroying)) {
      ...
      destroying = (async () => {
        ...
        await tryFinallyAsync2(
          () => tryFinallyAsync2(
            () => tryFinallyAsync2(stopAutoPersisting, () => actionToAwait),
            () => extraDestroy?.()
          ),
          () => {
...
var isCellOrValueSchemaTypes = (types) => isArray2(types) ? !isUndefined2(types[0]) && arrayEvery2(types, isCellOrValueSchemaType) : isCellOrValueSchemaType(types);
var cellOrValueSchemaTypeIncludes = (types, type) => isArray2(types) ? arrayHas(types, type) : types == type;
...
mapNew(arrayMap2(ENCODE, (char, index) => [char, index]));
...
var cloneSchema = (schema) => isArray2(schema) ? arrayMap2(schema, cloneSchema) : isObject2(schema) ? objMap(schema, cloneSchema) : schema;
...
var createStore = () => {
  ...
  const validateCellOrValueSchema = (schema) => {
    ...
    if (isUndefined2(enumValues) ? !isCellOrValueSchemaTypes(type) : !isUndefined2(type) || !isArray2(enumValues) || isUndefined2(enumValues[0]) || !arrayEvery2(enumValues, (enumValue) => {
  ...
  const cloneRow = (row) => isObject2(row) ? objMap(row, getArg) : row;
  const cloneTable = (table) => isObject2(table) ? objMap(table, cloneRow) : table;
  const cloneTables = (tables) => isObject2(tables) ? objMap(tables, cloneTable) : tables;
  const cloneValues = (values) => isObject2(values) ? objMap(values, getArg) : values;
  ...
  const validateRow = (tableId, rowId, row, skipDefaults) => {
    ...
    return objValidate(
      ...
      (cellId) => objHas2(rowWithDefaults, cellId)
    ));
  };
  ...
  const validateValues = (values, skipDefaults) => {
    ...
    return objValidate(
      ...
      (valueId) => objHas2(valuesWithDefaults, valueId)
    ));
  };
...
var useListenable2 = (listenable, thing, returnType, listenerArgGetters = EMPTY_LISTENER_ARG_GETTERS2) => {
  ...
  const getResult = (resolvedThing, listenerArguments) => resolvedThing?.[(returnType == 6 ? _HAS2 : GET2) + listenable]?.(...listenerArguments) ?? DEFAULTS2[returnType];
  const updateResult = (resolvedThing = getThing2(thing), listenerArguments = getListenerArguments()) => {
    const nextResult = getResult(resolvedThing, listenerArguments);
...
var argsOrGetArgs2 = (args, store, parameter) => arrayMap2(
  args,
  (arg) => isFunction2(arg) ? isEmpty2(arg) ? getThing2(arg) : arg(parameter, store) : arg
);
...
var getStoreCellComponentProps = (store, tableId) => ({
  get store() {
    return getValue2(store);
  },
  get tableId() {
    return getValue2(tableId);
  }
});
var getQueriesCellComponentProps = (queries, queryId) => ({
  get queries() {
    return getValue2(queries);
  },
  get queryId() {
    return getValue2(queryId);
  }
});
    ...
    return (() => {
      ...
      insert(
        ...
        () => arrayMap2(getValue2(rowIds), (rowId) => {
          const rowProps = mergeProps2(cellComponentProps, {
            rowId
          });
          return (() => {
            var _el$7 = _tmpl$5$3();
  ...
  return (() => {
    ...
    insert(
      ...
      (() => {
        ...
        return () => _c$() ? (() => {
          var _el$5 = _tmpl$2();
          addEventListener$1(_el$5, "click", props.handleEditable);
          effect(
            (_p$) => {
...
var Delete = (props) => {
  ...
  return [
    ...
    (() => {
      var _el$0 = _tmpl$5();
      addEventListener$1(_el$0, "click", props.onClick);
      return _el$0;
    })()
  ];
};
...
var ValuesView = (props) => {
  ...
  return Details({
    ...
    get children() {
      return [
        memo2(
          () => memo2(() => !!isEmpty2(valueIds()))() ? _tmpl$7() : createComponent2(ValuesInHtmlTable, {
            get store() {
              return props.store;
            },
            get editable() {
          ...
          get customCells() {
            return objNew(
              arrayMap2(cellIds(), (cellId2) => [
                ...
                {
                  label: cellId2,
                  component: editable() ? EditableCellViewWithActions : CellView
                }
              ])
            );
          }
...
var TablesView = (props) => {
  ...
  return Details({
    ...
    get children() {
      return [
        memo2(
          () => memo2(() => !!isEmpty2(tableIds()))() ? _tmpl$8() : sortedIdsMap(
            tableIds(),
...
var MetricsView = (props) => {
  ...
  return memo2(
    () => memo2(() => !!isUndefined2(metrics()))() ? EMPTY_STRING2 : createComponent2(Details, {
      ...
      get children() {
        return memo2(() => !!isEmpty2(metricIds()))() ? "No metrics defined" : (() => {
...
var IndexesView = (props) => {
  ...
  return memo2(
    () => memo2(() => !!isUndefined2(indexes()))() ? EMPTY_STRING2 : createComponent2(Details, {
      ...
      get children() {
        return memo2(() => !!isEmpty2(indexIds()))() ? "No indexes defined" : sortedIdsMap(
          indexIds(),
...
var QueriesView = (props) => {
  ...
  return memo2(
    () => memo2(() => !!isUndefined2(queries()))() ? EMPTY_STRING2 : createComponent2(Details, {
      ...
      get children() {
        return memo2(() => !!isEmpty2(queryIds()))() ? "No queries defined" : sortedIdsMap(
          queryIds(),
...
var RelationshipsView = (props) => {
  ...
  return memo2(
    () => memo2(() => !!isUndefined2(relationships()))() ? EMPTY_STRING2 : createComponent2(Details, {
      ...
      get children() {
        return memo2(() => !!isEmpty2(relationshipIds()))() ? "No relationships defined" : sortedIdsMap(
          relationshipIds(),
...
var Body = (props) => {
  ...
  return memo2(
    () => memo2(
      () => !!(state() && isUndefined2(store()) && isEmpty2(storeIds()) && isUndefined2(metrics()) && isEmpty2(metricsIds()) && isUndefined2(indexes()) && isEmpty2(indexesIds()) && isUndefined2(relationships()) && isEmpty2(relationshipsIds()) && isUndefined2(queries()) && isEmpty2(queriesIds()))
    )() ? (() => {
      var _el$30 = _tmpl$10();
