...
function Provider($$anchor, $$props) {
  ...
  let extras = $.proxy(arrayNew(8, () => ({})));
  const registrationsByOffset = arrayNew(8, () => mapNew());
  const setThing = (offset, id, thing) => {
    const thingsById = untrack(() => extras[offset]);
    if (objGet(thingsById, id) != thing) {
      extras[offset] = isUndefined(thing) ? objDel({ ...thingsById }, id) : { ...thingsById, [id]: thing };
    }
  };
  const addThing = (offset, id, thing, owner) => {
  ...
  const delThing = (offset, id, owner) => {
    const registrationsById = registrationsByOffset[offset];
    const registrations = arrayFilter(
      mapGet(registrationsById, id) ?? [],
      ([registrationOwner]) => registrationOwner != owner
