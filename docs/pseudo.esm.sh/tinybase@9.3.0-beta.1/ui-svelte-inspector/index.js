...
import { getContext as getContext2, onDestroy, onMount } from "https://esm.sh/svelte@^5.56.6";
import * as $2 from "https://esm.sh/svelte@^5.56.6/internal/client";
import "https://esm.sh/svelte@^5.56.6/internal/disclose-version";
import { createSubscriber as createSubscriber2 } from "https://esm.sh/svelte@^5.56.6/reactivity";

// dist/ui-svelte/index.js
import { getContext, setContext, untrack } from "https://esm.sh/svelte@^5.56.6";
import * as $ from "https://esm.sh/svelte@^5.56.6/internal/client";
import "https://esm.sh/svelte@^5.56.6/internal/disclose-version";
import { createSubscriber } from "https://esm.sh/svelte@^5.56.6/reactivity";
...
var string = String;
var boolean = Boolean;
var addEventListener = (target, event2, listener) => {
  target.addEventListener(event2, listener);
  return () => target.removeEventListener(event2, listener);
};
...
var isObject = (obj) => !isNullish2(obj) && ifNotNullish2(
  getPrototypeOf(obj),
  (objPrototype) => objPrototype == object2.prototype || isNullish2(getPrototypeOf(objPrototype)),
  /* istanbul ignore next */
  () => true
);
var objIds2 = object2.keys;
var objFreeze = object2.freeze;
var objHas = (obj, id2) => object2.hasOwn(obj, id2);
var objGet2 = (obj, id2) => ifNotUndefined2(
  obj,
  (obj2) => ifNotUndefined2(object2.getOwnPropertyDescriptor(obj2, id2), () => obj2[id2])
);
var objSet = (obj, id2, value) => {
  object2.defineProperty(obj, id2, {
    configurable: true,
    enumerable: true,
    value,
    writable: true
...
var objForEach = (obj, cb) => arrayForEach(objIds2(obj), (id2) => cb(obj[id2], id2));
var objEvery = (obj, cb) => {
...
var objIsEqual = (obj1, obj2, isEqual = (value1, value2) => value1 === value2) => objSize(obj1) === objSize(obj2) && objEvery(
  obj1,
  (value1, index2) => objHas(obj2, index2) ? isObject(value1) ? (
    /* istanbul ignore next */
    isObject(obj2[index2]) ? objIsEqual(obj2[index2], value1, isEqual) : false
  ) : isEqual(value1, obj2[index2]) : false
);
var objValidate = (obj, validateChild, onInvalidObj) => {
  if (isNullish2(obj) || !isObject(obj) || objIsEmpty(obj) || objFrozen(obj)) {
    onInvalidObj?.();
    return false;
  }
...
var rem = (...rems) => `${arrayJoin(rems, "rem ")}rem`;
var px = (...pxs) => `${arrayJoin(pxs, "px ")}px`;
...
var errorNew = (code, details) => new Error(
  TINYBASE2 + ":" + code + (isUndefined2(details) ? EMPTY_STRING2 : (
    /* istanbul ignore next */
    ":" + details
  ))
);
...
var collSizeN = (collSizer) => (coll) => {
  let total = 0;
  for (const child2 of coll.values()) {
    total += collSizer(child2);
  }
  return total;
};
...
var mapMatch = (map2, obj, set3, del = mapSet) => {
  objForEach(obj, (value, id2) => set3(map2, id2, value));
  ...
  const schedule = async (...actions) => {
    ...
    const completion = promiseNew(
      (resolve) => !isEmpty(actions) ? arrayPush(
        ...
        ...arrayMap(actions, (action, index2) => {
          const last = index2 == size(actions) - 1;
          return [
            async () => {
...
var createStoragePersister = (store, storageName, storage, onIgnoredError) => {
  ...
  const addPersisterListener = (listener) => addEventListener(WINDOW, STORAGE, (event2) => {
...
mapNew(arrayMap(ENCODE, (char, index2) => [char, index2]));
new GLOBAL2.TextEncoder();
...
var createStore = () => {
  ...
  const encodeValid = (cellOrValue, invalid) => ifNotUndefined2(
  ...
  ) : isUndefined2(getCellOrValueType(cell)) || isReservedString(cell, acceptingEncodedData) || isEncodedJson(cell) && !isValidEncodedJson(cell) ? cellInvalid(tableId, rowId, cellId, cell) : encodeValid(cell, () => cellInvalid(tableId, rowId, cellId, cell));
  ...
  const applyChanges = (changes) => fluentTransaction(
    () => ifTransformed(
      ...
      (changes2) => {
        objForEach(
          changes2[0],
          (table, tableId) => isUndefined2(table) ? delTable(tableId) : objForEach(
            table,
            (row, rowId) => isUndefined2(row) ? delRow(tableId, rowId) : objForEach(
              row,
  ...
  const rollbackTransaction = () => {
    ifNotUndefined2(oldTablesSchema, ([hasSchema, schema]) => {
      setValidTablesSchema(schema);
      hasTablesSchema = hasSchema;
    });
    ifNotUndefined2(oldValuesSchema, ([hasSchema, schema]) => {
...
var getSortedRowIds = (tableIdOrArgs, cellIdOrStoreOrStoreId, descending = false, offset = 0, limit, sorter, storeOrStoreId) => isObject(tableIdOrArgs) ? createListenable(
  resolveStore2(cellIdOrStoreOrStoreId),
  SORTED_ROW_IDS2,
  EMPTY_ARR,
  () => [tableIdOrArgs]
) : createListenable(
...
var root$m = $2.from_html(`<img tabindex="0" alt=""/>`);
...
var root$l = $2.from_html(`<img tabindex="0" alt=""/>`);
var root_1$c = $2.from_html(
...
$2.delegate(["click", "keydown"]);
var root$k = $2.from_html(`<button> </button>`);
var root_1$b = $2.from_html(`<input/>`);
var root_2$4 = $2.from_html(`<input type="number"/>`);
var root_3$3 = $2.from_html(`<input type="checkbox"/>`);
var root_4$2 = $2.from_html(`<div><!><!></div>`);
...
var root$j = $2.from_html(`<td><!></td>`);
var root_1$a = $2.from_html(`<th> </th> <th> </th>`, 1);
var root_2$3 = $2.from_html(`<tr><!><!><!><!></tr>`);
  ...
  $2.each(
    ...
    ($$anchor2, entry) => {
      ...
      const computed_const = $2.derived(() => {
        const [tableId, cellId] = strSplit($2.get(compoundCellId), ".", 2);
...
var root$i = $2.from_html(`<th> </th>`);
var root_1$9 = $2.from_html(`<th> </th> <th> </th>`, 1);
var root_2$2 = $2.from_html(`<thead><tr><!><!><!><!></tr></thead>`);
var root_3$2 = $2.from_html(`<table><!><tbody></tbody></table>`);
...
var root$h = $2.from_html(`<caption><!></caption>`);
var root_1$8 = $2.from_html(`<th> </th>`);
var root_2$1 = $2.from_html(`<th><!> Id</th>`);
var root_3$1 = $2.from_html(`<th><!> </th>`);
var root_4$1 = $2.from_html(`<thead><tr><!><!><!><!></tr></thead>`);
var root_5$1 = $2.from_html(`<td><!></td>`);
var root_6 = $2.from_html(`<tr><!><!><!><!></tr>`);
var root_7 = $2.from_html(`<table><!><!><tbody></tbody></table>`);
...
var root$g = $2.from_html(
  `<button class="previous"></button><button class="next"></button> `,
  1
);
var root_1$7 = $2.from_html(`<!> `, 1);
function SortedTablePaginator($$anchor, $$props) {
  $2.push($$props, true);
  $2.rest_props($$props, rest_excludes$3);
  const offset = $2.derived(
    () => $$props.offset == null || $$props.offset < 0 || $$props.offset > 0 && $$props.offset >= $$props.total ? 0 : $$props.offset
  );
  ...
  const rangeLabel = $2.derived(
    () => `${$2.get(offset) + 1} to ${mathMin($$props.total, $2.get(offset) + $2.get(limit))} of `
  );
  const totalLabel = $2.derived(
    () => `${$$props.total} ${$$props.total != 1 ? $2.get(plural) : $2.get(singular)}`
  );
  $2.user_effect(() => {
    const offset2 = $$props.offset ?? 0;
    if (offset2 < 0 || offset2 > 0 && offset2 >= $$props.total) {
      $$props.onChange(0);
    }
  });
  ...
  var node = $2.first_child(fragment);
  {
    var consequent = ($$anchor2) => {
      ...
      button.textContent = "\u2190";
      var button_1 = $2.sibling(button);
      button_1.textContent = "\u2192";
      var text3 = $2.sibling(button_1, 1, true);
      $2.template_effect(() => {
        button.disabled = $2.get(offset) == 0;
        button_1.disabled = $2.get(offset) + $2.get(limit) >= $$props.total;
        $2.set_text(text3, $2.get(rangeLabel));
      });
      $2.delegated(
        "click",
        button,
        () => $$props.onChange(mathMax(0, $2.get(offset) - $2.get(limit)))
      );
      $2.delegated(
        "click",
        button_1,
  ...
  var text_1 = $2.sibling(node, 1, true);
  $2.template_effect(() => $2.set_text(text_1, $2.get(totalLabel)));
  $2.append($$anchor, fragment);
  $2.pop();
}
$2.delegate(["click"]);
...
function ResultSortedTableInHtmlTable($$anchor, $$props) {
  $2.push($$props, true);
  let paginator = $2.prop($$props, "paginator", 3, false), props = $2.rest_props($$props, rest_excludes$2);
...
function SliceInHtmlTable($$anchor, $$props) {
  $2.push($$props, true);
  let props = $2.rest_props($$props, rest_excludes$1);
...
function SortedTableInHtmlTable($$anchor, $$props) {
  $2.push($$props, true);
  let paginator = $2.prop($$props, "paginator", 3, false), props = $2.rest_props($$props, rest_excludes);
  const defaultCellIds = getTableCellIds(
    () => $$props.tableId,
    () => $$props.store
  );
...
var root$f = $2.from_html(`<th> </th>`);
var root_1$6 = $2.from_html(`<th>Id</th>`);
var root_2 = $2.from_html(`<thead><tr><!><!><th> </th><!></tr></thead>`);
var root_3 = $2.from_html(`<td><!></td>`);
var root_4 = $2.from_html(`<tr><!><!><td><!></td><!></tr>`);
var root_5 = $2.from_html(`<table><!><tbody></tbody></table>`);
        ...
        Details($$anchor2, {
          ...
          children: ($$anchor3, $$slotProps) => {
            ...
            var node_1 = $2.first_child(fragment_2);
            {
              ...
              var d = $2.derived(() => isEmpty(indexIds.current));
...
var root$e = $2.from_html(`<tr><th> </th><td> </td><td><!></td></tr>`);
var root_1$5 = $2.from_html(
        ...
        Details($$anchor2, {
          ...
          children: ($$anchor3, $$slotProps) => {
            ...
            var node_1 = $2.first_child(fragment_2);
            {
              ...
              var d = $2.derived(() => isEmpty(metricIds.current));
              var alternate = ($$anchor4) => {
                var table = root_1$5();
        ...
        Details($$anchor2, {
          ...
          children: ($$anchor3, $$slotProps) => {
            ...
            var node_1 = $2.first_child(fragment_2);
            {
              ...
              var d = $2.derived(() => isEmpty(queryIds.current));
              var alternate = ($$anchor4) => {
                var fragment_3 = $2.comment();
                var node_2 = $2.first_child(fragment_3);
                $2.each(
        ...
        Details($$anchor2, {
          ...
          children: ($$anchor3, $$slotProps) => {
            ...
            var node_1 = $2.first_child(fragment_2);
            {
              ...
              var d = $2.derived(() => isEmpty(relationshipIds.current));
              var alternate = ($$anchor4) => {
                var fragment_3 = $2.comment();
                var node_2 = $2.first_child(fragment_3);
                $2.each(
...
var root$c = $2.from_html(` <img title="Confirm" class="ok" alt=""/>`, 1);
function Delete($$anchor, $$props) {
  ...
  $2.user_effect(() => {
    if (hasWindow2()) {
      return addEventListener(document, KEYDOWN, (event2) => {
        if (event2.key == "Enter") {
          event2.preventDefault();
          handleClick();
        }
...
$2.delegate(["click"]);
var root$b = $2.from_html(` <input type="text"/>  <img alt=""/>`, 1);
...
var root$a = $2.from_html(`<!>  <img title="Cancel" class="cancel" alt=""/>`, 1);
var root_1$4 = $2.from_html(`<img alt=""/>`);
  ...
  );
  {
    ...
    const right = ($$anchor2) => {
      ...
      var node = $2.first_child(fragment_2);
      {
        ...
        var d = $2.derived(() => !isEmpty($2.get(rightActions)));
        $2.if(node, ($$render) => {
          if ($2.get(d)) $$render(consequent);
        });
      }
...
var root$7 = $2.from_html(`<p>No tables.</p>`);
  ...
  Details($$anchor, {
    ...
    children: ($$anchor2, $$slotProps) => {
      ...
      var node = $2.first_child(fragment_1);
      {
        ...
        var d = $2.derived(() => isEmpty(tableIds.current));
        var alternate = ($$anchor3) => {
          var fragment_2 = $2.comment();
          var node_1 = $2.first_child(fragment_2);
          $2.each(
...
var root$6 = $2.from_html(`<p>No values.</p>`);
var root_1$2 = $2.from_html(`<!> <!>`, 1);
  ...
  Details($$anchor, {
    ...
    children: ($$anchor2, $$slotProps) => {
      ...
      var node = $2.first_child(fragment_1);
      {
        ...
        var d = $2.derived(() => isEmpty(valueIds.current));
        var alternate = ($$anchor3) => {
          {
...
var root$4 = $2.from_html(`<span class="warn"> </span>`);
var root_1$1 = $2.from_html(
  `<article><!> <!> <!> <!> <!> <!> <!> <!> <!> <!></article>`
);
function Body($$anchor, $$props) {
  ...
  const noProvidedObjects = $2.derived(
    () => isUndefined2(store) && isEmpty(storeIds.current) && isUndefined2(metrics) && isEmpty(metricsIds.current) && isUndefined2(indexes) && isEmpty(indexesIds.current) && isUndefined2(relationships) && isEmpty(relationshipsIds.current) && isUndefined2(queries) && isEmpty(queriesIds.current)
  );
  var fragment = $2.comment();
  var node = $2.first_child(fragment);
  {
...
var root$3 = $2.from_html(`<span class="warn"> </span>`);
...
var root$2 = $2.from_html(`<img tabindex="0" alt=""/>`);
var root_12 = $2.from_html(
...
$2.delegate(["click", "keydown"]);
var root$12 = $2.from_html(`<main><!> <!></main>`);
