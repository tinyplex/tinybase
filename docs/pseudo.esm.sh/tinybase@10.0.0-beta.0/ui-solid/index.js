...
var Provider = (props) => {
  ...
  const extraThingRegistrations = arrayNew(8, () => mapNew());
  const setExtraThingById = (thingOffset, id, thing) => {
    setExtraThingsById((extraThingsById2) => {
      if (objGet(extraThingsById2[thingOffset], id) == thing) {
        return extraThingsById2;
      }
      const thingsById = isUndefined(thing) ? objDel(
        {
      ...
      return arrayWith(extraThingsById2, thingOffset, thingsById);
    });
  };
  const addExtraThingById = (thingOffset, id, thing, owner) => {
  ...
  const delExtraThingById = (thingOffset, id, owner) => {
    const registrationsById = extraThingRegistrations[thingOffset];
    const registrations = arrayFilter(
      mapGet(registrationsById, id) ?? [],
      ([registrationOwner]) => registrationOwner != owner
    ...
    mapSet(
      registrationsById,
      id,
      isEmpty(registrations) ? void 0 : registrations
    );
    setExtraThingById(
      thingOffset,
      id,
...
var useListener = (listenable, thing, listener, preListenerArgGetters = EMPTY_LISTENER_ARG_GETTERS, ...postListenerArgGetters) => {
  if (!isServer) {
    createRenderEffect(
      () => onCleanup(
        addAndDelListener(
...
var useUndoOrRedoInformation = (checkpointsOrCheckpointsId, undo) => {
  const checkpoints = useCheckpointsOrCheckpointsById(
    checkpointsOrCheckpointsId
  );
  const checkpointIds = useCheckpointIds(checkpoints);
  const checkpointId = () => undo ? checkpointIds()[1] : checkpointIds()[2][0];
...
var useUndoInformation = (checkpointsOrCheckpointsId) => useUndoOrRedoInformation(checkpointsOrCheckpointsId, true);
var useRedoInformation = (checkpointsOrCheckpointsId) => useUndoOrRedoInformation(checkpointsOrCheckpointsId);
...
var useCreateSynchronizer = (store, create, destroy) => {
  ...
  createEffect(() => {
    ...
    const resolvedStore = getThing(store);
    const destroySynchronizer = (synchronizer2) => tryFinallyAsync(
...
var RemoteRowView = (props) => {
  ...
  const content = () => {
    ...
    return wrap(
      isUndefined(remoteTableId) || isUndefined(remoteRowId) ? null : createComponent(
        Row,
        mergeProps(
