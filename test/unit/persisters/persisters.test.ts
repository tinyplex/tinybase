...
import {noop, pause, waitFor, withServers} from '../common/other.ts';
...
if (!withServers) {
  ...
  test('awaits only its running action on destroy', async () => {
    ...
    const destroying = persister1
      .destroy()
      .then(() => (destroyResolved = true));
  ...
  test.each([false, true])(
    ...
    async (startSaveFirst) => {
      ...
      expect(persister.isAutoSaving()).toBe(false);
      expect(store.getListenerStats().transaction).toBe(0);
      expect(delPersisterListener).toHaveBeenCalledTimes(
        startSaveFirst ? 0 : 1,
      );
      expect(
