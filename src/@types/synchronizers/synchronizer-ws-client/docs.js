...
}
{
   ...
   *
   * The WsSynchronizer owns the WebSocket and closes it when it is destroyed.
   * Instead of the raw browser implementation of WebSocket, you may prefer to
   * use the [Reconnecting
   ...
   *
   * This method is asynchronous because it will await the websocket's
   * connection to the server. You will need to `await` a call to this function
   ...
   * increasing the target when needed. When set, larger synchronization
   * payloads are split into fragments and reassembled by the receiving
  ...
  /**
   * This overload creates a WsSynchronizer that shares one WebSocket with
   * other WsSynchronizer instances, each synchronizing a different
   * MergeableStore.
   *
   ...
   * destroyed. Reconnecting WebSocket implementations will renegotiate and
   * resubscribe all channels, then flush queued traffic.
   *
   ...
   * WebSocket message fragment. Unicode code points are never split and can
   * exceed this size. TinyBase sends at most 1,000 fragments for one payload,
   * increasing the target when needed.
   * @returns A reference to the new WsSynchronizer object.
   * @example
