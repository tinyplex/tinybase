...
![Todo App v3 (persistence)](/shots/todo-app-v3-demo.png)
...
each Cell present in each Table. Here we will indicate that the `text` Cell is a
string, and we default the `done` field to `false`. The `type` Cell uses an
`enum` rather than a `type`, so that it can only ever be one of the values of the
`TYPES` array, and anything else is replaced with the `'Home'` default:
...
-  );
+  const viewStore = useCreateStore(() =>
+    createStore().setValuesSchema({type: {enum: TYPES, default: 'Home'}}),
+  );
+  useCreatePersister(
+    viewStore,
