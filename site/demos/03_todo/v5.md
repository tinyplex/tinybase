...
![Todo App v5 demo thumbnail](/shots/todo-app-v5-demo.png "Thumbnail")
...
+   ({target: {value}}) => {
+     setType(value);
+     checkpoints.addCheckpoint(`changing to '${value}'`);
+   },
+   [checkpoints, setType],
+ );
...
+  const text = useCell(tableId, rowId, 'text');
+  const addCheckpoint = useSetCheckpointCallback(
