...
![Todo App v2 demo thumbnail](/shots/todo-app-v2-demo.png "Thumbnail")
...
We use the useValueState hook to both read the current type and get a setter
function to update it when clicked:
...
components completely self-sufficient. (In a more complex app, you might want to
optimize this by having the parent `Types` component listen for changes to the
current type and pass that down as a prop to each `Type` component instead.)
