...
![Todo App v1 demo thumbnail](/shots/todo-app-v1-demo.png "Thumbnail")
...
that toggles the `done` flag when clicked. We use the useCellState hook to both
read the current `done` state and get a setter function to toggle it:
...
that prop here, but instead we're using the useCellState hook that will
