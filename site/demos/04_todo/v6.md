...
the WebSocket messages between multiple clients.
TinyBase provides a ready-made server for this purpose called WsServer,
available in the synchronizer-ws-server module.
...
This is a simple change to the hook that creates the Store:
...
A MergeableStore is fully compatible with the Store API, so there are no other
changes required within the app to accommodate this.
...
We need to add in the new Store creation function as described above:
...
For example, one set of people might collaborate on a todo
list brokered by `wss://todo.demo.tinybase.org/1234`, others on a todo
list brokered by `wss://todo.demo.tinybase.org/5678` - and so on.
...
There are many more sophisticated (and secure!) ways to do this, but we are
going for a simple approach of using the URL query string to store a random path.
We use a hook to store the path in the App's state, and which gets the initial
value. It also provides a function that creates a new room and updates the URL
accordingly.
...
const useServerPathId = () => {
  ...
  return [
    serverPathId,
    useCallback(() => {
      const newServerPathId = ('' + Math.random()).substring(2, 12);
...
you should instead implement or import a true UUID generator (and perhaps
incorporate the user's authenticated identity!) if using this in a real
application.

Note also that we work with the `parent` location, rather than the `window`
object. This is simply because the TinyBase demo runs in a trusted iframe and
needs to get the URL from the outer page. Fortunately `parent` still resolves to
`window` even when this _isn't_ running in an iframe.
...
Note that we are still persisting the data locally to local storage.
...
Synchronizer is created. This will start the synchronization.
