...
There is a simple change to the hook that creates the Store with a unique Id:

```diff-js
-const store = useCreateStore(() => createStore().setTablesSchema(SCHEMA));
+const store = useCreateMergeableStore(() => {
+  const storeId = ('' + Math.random()).substring(2, 12);
+  return createMergeableStore(storeId).setTablesSchema(SCHEMA);
+});
```

(In a real production app you would want to generate a better unique ID than
just a random number, but this is fine for the purposes of a demo.)

Since a MergeableStore is fully compatible with the Store API, there are no
other changes required within the app to accommodate this.
...
going for a simple approach of using the URL query string to store a random
path. We use a hook to store the path in the App's state, and which gets the
initial value. It also provides a function that creates a new room and updates
the URL accordingly.
